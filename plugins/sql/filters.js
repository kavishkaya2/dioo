...
async function getFilter(jid = null, filter = null) {
    var Wher = { chat: jid };
    if (filter !== null) Wher.push({ queeendiacmd: filter });
...
async function setFilter(jid = null, filter = null, tex = null, regx = false) {
    ...
    if (Msg.length < 1) {
        return await FiltersDB.create({ chat: jid, queeendiacmd: filter, text: tex, regex: regx });
    } else {
        return await Msg[0].update({ chat: jid, queeendiacmd: filter, text: tex, regex: regx });
    }
}
