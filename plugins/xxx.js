...
const thumb = fs.readFileSync('./uploads/thumb/thumbdia.jpg') 
    diana.addqueendiacmd({ queeendiacmd: 'video ?(.*)', fromMe: tk, deldiacmd: false, desc: "Download videos",  deldiacmd: false}, async (message, match) => {
        const linkk = match[1]
    ...
    diana.addqueendiacmd({ queeendiacmd: 'video ?(.*)', fromMe: true, deldiacmd: false, desc: "Download videos",  deldiacmd: false}, async (message, match) => {
        const linkk = match[1]
        if (!linkk) return await message.client.sendMessage(message.jid,YT_NEED,MessageType.text)
        await message.client.sendMessage(message.jid,DWLOAD_VID,MessageType.text ,  {quoted: message.data});
        await axios
