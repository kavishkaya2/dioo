...
if (CON.WORKTYPE == 'private') {
    diana.addqueendiacmd({ queeendiacmd: 'whois$', fromMe: true, desc: Lang.PL_DESC }, async(message, match) => {
        if (message.jid.includes('-')) {
            ...
            await message.sendMessage(
                ...
                MessageType.image, { caption: nwmsg }
            );
        }
    });
} else if (CON.WORKTYPE == 'public') {
    diana.addqueendiacmd({ queeendiacmd: 'whois$', fromMe: false, desc: Lang.PL_DESC }, async(message, match) => {
        if (message.jid.includes('-')) {
            var json = await message.client.groupMetadataMinimal(message.jid)
            var code = await message.client.groupInviteCode(message.jid)
            var nwjson = await message.client.groupMetadata(message.jid)
