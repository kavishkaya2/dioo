...
//                               MAIN COMMAND
// -----------------------------------------------------------------------
diana.addqueendiacmd({ queeendiacmd: 'afb ?(.*)', fromMe: ramiya, desc: 'mada' }, async(message, match) => {
...
diana.addqueendiacmd({ queeendiacmd: 'afb ?(.*)', fromMe: teenuh, desc: 'mada' }, async(message, match) => {
        // අමාරුවෙන් යකො හැදුවෙ. උස්සන්න එපා තිරිසනෝ 😒
        const link = match[1]
...
diana.addqueendiacmd({ queeendiacmd: 'xxfbsd ?(.*)', fromMe: ramiya, noinmenu: true }, async(message, match) => {
        // අමාරුවෙන් යකො හැදුවෙ. උස්සන්න එපා තිරිසනෝ 😒
        const link = match[1]
