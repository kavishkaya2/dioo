...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to diana voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
diana.addqueendiacmd({ on: 'text', fromMe: wk, noinmenu: true, deldiacmd: false }, (async(message, match) => {
    if (message.message.startsWith('Diana') && conf.TALKING_DIANA !== 'true') {
        var unique_ident = message.client.user.jid.split('@')[0]
        let acc = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0] == 'Diana' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
        let aitalk_mode = message.message.includes('{normal}') ? 'raw' : 'waifu'
        var finm = message.message.replace('Diana', '').replace(' ', '')
        var ainame = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0]
        if (ainame !== 'Diana') return;
...
diana.addqueendiacmd({ on: 'text', fromMe: false, deldiacmd: false }, (async(message, match) => {
    if (conf.TALKING_DIANA == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') &&
...
diana.addqueendiacmd({ queeendiacmd: 'vtalk$', desc: vtalk_dsc, noinmenu: true, fromMe: wk }, (async(message, match) => {
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Diana özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Diana yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Diana yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Diana, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Diana, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional diana features. Turn your account into a ai chatbot!'
    already_on = 'diana artificial intelligence is already fully functional.'
    already_off = 'diana artificial intelligence is currently running semi-functional.'
    succ_on = 'diana Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'diana Set to Semi-Functional! Please wait a bit! ☑️'
}
...
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Diana സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Diana കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Diana AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Diana പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Diana സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

diana.addqueendiacmd({ queeendiacmd: 'talkingdiana ?(.*)', desc: fulleva_dsc, fromMe: true, noinmenu: true, usage: '.Diana on / off' }, (async(message, match) => {
    var pinky_status = `${conf.TALKING_DIANA}`
    if (match[1] == 'on') {
