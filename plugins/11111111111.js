...
let wk = Config.WORKTYPE == 'public' ?  : true


diana.addqueendiacmd({queeendiacmd: 'nrainbow ?(.*)', fromMe: wk, noinmenu: true}, (async (message, match) => {
    thiccysapi.textpro("https://textpro.me/3d-rainbow-color-calligraphy-text-effect-1049.html",
        `${match[1]}`
          ...
          try { 
              ...
              await download(`${data}`, '/root/QUEENDIANA/crainbow.jpg', async() => {                          
                  await message.client.sendMessage(message.jid,fs.readFileSync('/root/QUEENDIANA/crainbow.jpg'), MessageType.image, { caption: Config.AFN })
              })
          } catch(err) { 
              console.log(err)
          } 
