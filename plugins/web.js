...
diana.addqueendiacmd({queeendiacmd: '?(.*)', fromMe: true, deldiacmd: false, noinmenu: true}, (async (message, match) => {
    var HANDLER = '';
...
diana.addqueendiacmd({ queeendiacmd: 'speedtest', fromMe: true, desc: Lang.SPEEDTEST_DESC }, (async(message, match) => {
...
diana.addqueendiacmd({ queeendiacmd: 'ping$', fromMe: true, deldiacmd: false, desc: Lang.PING_DESC }, (async(message, match) => {
...
if (Config.WORKTYPE == 'private') {
    ...
    diana.addqueendiacmd({ queeendiacmd: 'short ?(.*)', fromMe: true, desc: Lang.URL }, (async(message, match) => {
    ...
    diana.addqueendiacmd({ queeendiacmd: 'calc ?(.*)', fromMe: true, desc: Lang.CALC }, (async(message, match) => {
        ...
        if (match[1].includes('+')) {
            ...
            try { await message.client.sendMessage(message.jid, Lang.SUC + result, MessageType.text) } catch (err) {
    ...
    diana.addqueendiacmd({ queeendiacmd: 'short ?(.*)', fromMe: false, desc: Lang.URL }, (async(message, match) => {
    ...
    diana.addqueendiacmd({ queeendiacmd: 'calc ?(.*)', fromMe: false, desc: Lang.CALC }, (async(message, match) => {
        if (match[1].length < 4) { return await message.client.sendMessage(message.jid, Lang.VALİD, MessageType.text) }
        if (match[1].includes('+')) {
            var split = match[1].split('+'),
                sonsayi = split[1],
