...
if (Config.LANG == 'ML') {
    ...
    plk9 = ' https://github.com/kavishkaya/BotDIAnaWhatsapp '
} //afnanplk creation
else {
    plk1 = 'ᴛᴏ ɢᴇᴛ ᴀʟʟ ᴄᴏᴍᴍᴀɴᴅꜱ ᴛʏᴘᴇ .DIAna'
    plk2 = 'hey am '
...
diana.addqueendiacmd({ on: 'text', fromMe: false, grponly: true, deldiacmd: false }, (async(message, match) => {
    if (AI_var == 'group' && message.jid !== '919562803423-1627735504@g.us') {
        ...
        let regex10 = new RegExp(DIAna.OA_NAME)
        if (regex1.test(message.message)) {
            await message.client.sendMessage(message.jid, plk1, MessageType.text, { quoted: message.data })
        } else if (regex2.test(message.message)) {
            await message.client.sendMessage(message.jid, plk2 + DIAna.OB_NAME, MessageType.text, { quoted: message.data })
        } else if (regex3.test(message.message)) {
            ...
            await message.client.sendMessage(message.jid, DIAna.OA_REPLY, MessageType.text, { quoted: message.data })
        }
    }
}));
...
diana.addqueendiacmd({ on: 'text', fromMe: false, inboxonly: true, deldiacmd: false }, (async(message, match) => {
    if (AI_var == 'pm' && message.jid !== '919562803423-1627735504@g.us') {

        let regex1 = new RegExp('command ')
        let regex2 = new RegExp('name ')
        let regex3 = new RegExp('bot ')
        let regex4 = new RegExp('Diana ')
