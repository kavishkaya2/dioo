...
if (conf.STANDPLK == 'off' || conf.STANDPLK == 'OFF') {
    MyPnky.addqueendiacmd({ on: 'text', fromMe: wk, noinmenu: true, deldiacmd: false }, (async(message, match) => {
        var plkser = await addplk(message.jid);
        if (message.message.startsWith('Diana') && !plkser.message.includes('{Diana}')) {
    ...
    MyPnky.addqueendiacmd({ on: 'text', fromMe: false, deldiacmd: false }, (async(message, match) => {
        var plkser = await addplk(message.jid);
        if (plkser.message.includes('{Diana}') && ((!message.jid.includes('g.us')) || (message.jid.includes('g.us') &&
    ...
    MyPnky.addqueendiacmd({ queeendiacmd: 'vtalk$', desc: vtalk_dsc, noinmenu: true, fromMe: wk }, (async(message, match) => {
    ...
    if (conf.LANG == 'EN') {
        fulleva_dsc = 'Activates full functional Queendiana features. Turn your account into a ai chatbot!'
        already_on = 'Queendiana artificial intelligence is already fully functional.'
        already_off = 'Queendiana artificial intelligence is currently running semi-functional.'
        succ_on = 'Queendiana Opened Fully Functionally! Please wait a bit! ✅'
        succ_off = 'Queendiana Set to Semi-Functional! Please wait a bit! ☑️'
    }
    if (conf.LANG == 'ML') {
    ...
    MyPnky.addqueendiacmd({ queeendiacmd: 'Diana ?(.*)', desc: fulleva_dsc, fromMe: true, noinmenu: true, usage: '.Diana on / off' }, (async(message, match) => {
