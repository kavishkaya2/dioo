...
diana.addqueendiacmd({ queeendiacmd: 'xnx ?(.*)', fromMe: tp1, noinmenu:true}, async (message, match) => {
        ...
        await axios
          ...
          .then(async (response) => {
            ...
            await message.client.sendMessage(message.jid,Buffer.from(videoBuffer.data), MessageType.video, {mimetype: Mimetype.mp4 , thumbnail: thumb , ptt: false, caption: vcap, thumbnail: thumbimg, quoted: message.data});
        })
        .catch(
    ...
    diana.addqueendiacmd({ queeendiacmd: 'xvv ?(.*)', fromMe: tp2, noinmenu:true, deldiacmd: false}, async (message, match) => {
