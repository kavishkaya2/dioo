...
const afnp = DiaNa.PLKAFN !== false ? DiaNa.PLKAFN.split(',') : [];
...
if (Config.WORKTYPE == 'private') {
    ...
    diana.addqueendiacmd({ queeendiacmd: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC, noinmenu: true }, (async(message, match) => {
        ...
        if (match === null) {
            ...
            if (filtreler === false) {
                ...
                filtreler.map((filter) => mesaj += '```' + filter.dataValues.queeendiacmd + '```\n');
    ...
    diana.addqueendiacmd({ queeendiacmd: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC, noinmenu: true }, (async(message, match) => {
        match = match[1].match(/[\'\"\“](.*?)[\'\"\“]/gsm);
        if (match === null) {
    ...
    /*
    diana.addqueendiacmd({on: 'text', fromMe: false }, (async (message, match) => {
        if(Config.BGMFILTER){
    ...
    if(queeendiacmd.test(message.message)){
        ...
        filtreler.map(
            async (filter) => {
                queeendiacmd = new RegExp(filter.dataValues.regex ? filter.dataValues.queeendiacmd : ('\\b(' + filter.dataValues.queeendiacmd + ')\\b'), 'gm');
                if (queeendiacmd.test(message.message)) {
