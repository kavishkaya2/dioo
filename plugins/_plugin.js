...
let msg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*Bu Plugin Resmi Olarak Onaylanmıştır!* ✅' : '*This Plugin is Officially Approved by Diana!* ✅'
let unmsg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*Bu Plugin Resmi Değildir!* ❌' : '*Diana not accptt this Plugin please remove this!* ❌'
...
diana.addqueendiacmd({ queeendiacmd: 'pkg ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, noinmenu: true }, (async(message, match) => {
    ...
    if (response.statusCode == 200) {
        // Plugin Name
        var plugin_name = response.body.match(/addqueendiacmd\({.*queeendiacmd: ["'](.*)["'].*}/);
...
diana.addqueendiacmd({ queeendiacmd: 'plugin$', fromMe: true, noinmenu: true, desc: Lang.PLUGIN_DESC }, (async(message, match) => {
...
diana.addqueendiacmd({ queeendiacmd: 'remove(?: |$)(.*)', fromMe: true, noinmenu: true, desc: Lang.REMOVE_DESC }, (async(message, match) => {
