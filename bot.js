...
async function DIANA() {
    await config.DATABASE.sync();
    var StrSes_Db = await DIANADB.findAll({
        where: {
            info: 'StringSession'
        }
    });
    ...
    conn.on('credentials-updated', async() => {
        ...
        if (StrSes_Db.length < 1) {
            await DIANADB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
    ...
    conn.on('connecting', async() => {
        console.log(`${chalk.green.bold('Queen')}${chalk.blue.bold('Diana')}
    ...
    conn.on('open', async() => {
        ...
        if (config.LANG == 'EN' || config.LANG == 'SI') {
            ...
            if (commits.total === 0) {
                ...
                await conn.sendMessage(conn.user.jid, "👸𝚀𝚄𝙴𝙴𝙽 𝙳𝙸𝙰𝙽𝙰👸 ɪꜱ ᴀʟʟ ꜱᴇᴛ", MessageType.text);
                await conn.sendMessage(conn.user.jid, "```👸𝚀𝚄𝙴𝙴𝙽 𝙳𝙸𝙰𝙽𝙰👸 WORKING " + config.WORKTYPE + "```", MessageType.text);

                //======================test to my number
                /*   await conn.sendMessage(config.LOGSETTINGS, '\n\n\n\n' + Lang.UPDATE +'\n\n\n\n\n\n *⚡powerd by diana*', MessageType.text);
                    await conn.sendMessage(config.LOGSETTINGS, "Diana ɪꜱ ᴀʟʟ ꜱᴇᴛ", MessageType.text);
                    await conn.sendMessage(config.LOGSETTINGS, "```Diana WORKING " + config.WORKTYPE + "```" , MessageType.text);
...
//============================ebd */
                ...
                await conn.sendMessage(conn.user.jid, "👸𝚀𝚄𝙴𝙴𝙽 𝙳𝙸𝙰𝙽𝙰 FIXING👸", MessageType.text);
                await conn.sendMessage(conn.user.jid, "```👸𝚀𝚄𝙴𝙴𝙽 𝙳𝙸𝙰𝙽𝙰👸 WORKING " + config.WORKTYPE + "```", MessageType.text);
                //==================================================
        ...
        events.commands.map(
            async(command) => {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo') &&
                        ...
                        (command.queeendiacmd === undefined || (command.queeendiacmd !== undefined &&
                            command.queeendiacmd.test(text_msg)))) ||
                    (command.queeendiacmd !== undefined && command.queeendiacmd.test(text_msg)) ||
                    ...
                    if ((config.SUDO !== false && msg.key.fromMe === false && command.fromMe === true &&
                        ...
                        if (!command.inboxonly === chat.jid.includes('-')) sendMsg = true;
                        else if (command.grponly === chat.jid.includes('-')) sendMsg = true;
                    ...
                    if (sendMsg) {
                        ...
                        var match = text_msg.match(command.queeendiacmd);
                        ...
                        /*
                                                if (command.deldiacmd && msg.key.fromMe) {
                                                    await whats.delete(); 
                                                }
                        */
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '-- HATA RAPORU [DIANA] --' +
                                    '\n*DIANA bir hata gerçekleşti!*' +
                                    '\n_Bu hata logunda numaranız veya karşı bir tarafın numarası olabilir. Lütfen buna dikkat edin!_' +
                                    '\n_Yardım için Telegram grubumuza yazabilirsiniz._' +
                                    '\n_Bu mesaj sizin numaranıza (kaydedilen mesajlar) gitmiş olmalıdır._\n\n' +
                                    'Gerçekleşen Hata: ' + error + '\n\n', MessageType.text);
                            } else {
                                await conn.sendMessage(conn.user.jid, '__Diana_☠☠_[error] ' +
                                    '\n\n*👻 ' + error + '*\n', MessageType.text);



                                await conn.sendMessage(config.LOGSETTINGS, '__Diana_☠☠_[error] ' +
                                    '\n\n*👻 ' + error + '*\n', MessageType.text);
                            }
                        }
                    }
